...
class ModifyRelicDialog:
    ...
    def _update_illegal_status_display(self, relic_id, effects):
        ...
        # FIRST: Check if there's a valid ordering - if so, just say "needs reordering"
        if relic_checker:
            valid_order = relic_checker.get_valid_order(relic_id, effects)
            if valid_order:
                reasons.append("• Effects are valid but in the WRONG ORDER")
                reasons.append("• Use 'Find Valid ID' button or 'Mass Fix' to automatically reorder")
                self.status_label.config(text="❌ ILLEGAL (fixable)", foreground='#CC6600')  # Orange-red
        ...
        for i, eff in enumerate(effects[:3]):
            ...
            if pool_id == -1:
                ...
                if eff not in pool_effects:
                    # Effect CAN exist on relic, just in wrong slot - but no valid ordering found
                    reasons.append(f"• Effect '{eff_name}' is in wrong slot and no valid ordering exists")
